...
func main() {
	...
	eventQueueName := os.Getenv("EVENT_QUEUE_NAME")
	if eventQueueName == "" {
		eventQueueName = "task_events"
	}
	...
	jobDLQName := os.Getenv("JOB_DLQ_NAME")
	if jobDLQName == "" {
		jobDLQName = "task_jobs_dlq"
	}

	eventPublisher := tasksamqp.NewPublisher(rabbitURL, eventQueueName, "tasks", "v1", log)
	defer eventPublisher.Close()

	jobPublisher := tasksamqp.NewJobPublisher(rabbitURL, jobQueueName, jobDLQName, log)
	defer jobPublisher.Close()

	taskService := service.NewTaskService(repo, redisClient, log, cacheTTL, cacheTTLJitter, eventPublisher, jobPublisher)
