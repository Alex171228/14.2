...
type JobPublisher interface {
	PublishJob(ctx context.Context, job jobs.TaskJob) error
}
...
func NewTaskService(repo TaskRepository, redisClient *redis.Client, log *zap.Logger, ttl, ttlJitter time.Duration, eventPublisher TaskEventPublisher, jobPublisher JobPublisher) *TaskService {
	return &TaskService{
		repo:           repo,
		redis:          redisClient,
		log:            log,
...
func (s *TaskService) Create(ctx context.Context, req CreateTaskRequest) (*Task, error) {
	...
	if s.eventPublisher != nil {
		if err := s.eventPublisher.PublishTaskCreated(ctx, task.ID); err != nil {
...
func (s *TaskService) EnqueueProcessTask(ctx context.Context, taskID string) (*jobs.TaskJob, error) {
	if taskID == "" {
		return nil, errors.New("task id is required")
	}
	if s.jobPublisher == nil {
		return nil, errors.New("job publisher is not configured")
	}

	job := jobs.TaskJob{
		Job:       jobs.DefaultJobName,
		TaskID:    taskID,
		Attempt:   1,
